...
Appendix A contains all 69 hypotheses as historical records. Note: H2 was initially validated against synthetic ln_ST targets; real-data validation is recorded in H12 (Cycle 6).
...
| H64 | Brunnian 安定性ルールの唯一性証明 | 25 | accepted | FPR = 0.0 |
| H65 | クォーク(10.0)係数の第一原理導出 | 25 | accepted | error<0.04% |
...
## Revision Notes (Revision 3)

- **[R-1] Observation Count Consistency**: Synchronized Section 4.1 text ("9 fermions") with the free parameter table (updated to "Observations: 9", "Ratio: 4:9") to resolve the contradiction.
- **[R-2] η_quark Update in Appendix**: Corrected the H65 name in Appendix A to "クォーク(10.0)係数の第一原理導出" to match the body of the paper and SSoT `constants.json`.
- **[R-3] Synthetic Data Caveat**: Added a clear note at the start of Appendix A and an asterisk to the H2 entry explaining its reliance on synthetic data, as cautioned in SSoT.
- **[Misc] Precision & Terminology**: Updated H53 precision to "0.0000263%" and H64 FPR to "0.0" in Appendix A for consistency. Refined the concluding sentence to use "topological phase space of the SM spectrum" per reviewer recommendation.

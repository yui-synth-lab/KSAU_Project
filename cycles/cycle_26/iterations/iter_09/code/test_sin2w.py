...
def main():
    ...
    sin2w = consts['physical_constants']['sin2theta_w_geometric']
    # If q = exp(2pi i * sin2w), and x = q^(1/2) for links
    x_val = np.exp(1j * np.pi * sin2w)
    ...
    for rule_name, rule_func in rules.items():
        ...
        for i, u in enumerate(up_types):
            for j, d in enumerate(down_types):
                v_complex[i, j] = rule_func(quark_z[u], quark_z[d])
        ...
        print(f"Rule {rule_name}: {best_match}/9")

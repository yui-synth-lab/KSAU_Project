...
def main():
    ...
    rules = {
        ...
        'Z_u * conj(Z_d)': lambda u, d: u * np.conj(d),
        '1 / (Z_u - Z_d)': lambda u, d: 1.0 / (u - d),
        'Re(Z_u) / Re(Z_d)': lambda u, d: u.real / d.real + 0j,
        'Im(Z_u) / Im(Z_d)': lambda u, d: u.imag / d.imag + 0j
    }
    
    found = False
    for n in range(2, 121):
        for mode in ['pi/n', '2pi/n']:
            x_val = np.exp(1j * np.pi / n) if mode == 'pi/n' else np.exp(2j * np.pi / n)
            for rule_name, rule_func in rules.items():
                ...
                # Filter out zeros
                if np.any(np.abs(v_complex) < 1e-15): continue
                        ...
                        for c1 in [1, -1]:
                            for c2 in [1, -1]:
                                for c3 in [1, -1]:
                                    ...
                                    if match == 9:
                                        print(f"FOUND 100% MATCH: n={n}, mode={mode}, rule={rule_name}")
                                        print(derived_signs)
                                if found: break
                            if found: break
                        if found: break

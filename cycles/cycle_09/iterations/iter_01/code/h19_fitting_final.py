...
def main():
    ...
    # x0: etas, intercepts, alpha, beta, gamma_links
    # Manual guess based on calculations
    x0 = []
    for s in sectors:
        if s == "quarks_c2": x0.extend([8.0, -5.0])
        elif s == "quarks_c3": x0.extend([5.4, -3.5])
        elif s == "leptons": x0.extend([17.5, 0.0])
        elif s == "bosons": x0.extend([3.0, 5.5])
        else: x0.extend([1.0, 0.0])
        
    x0.extend([0.1, 0.1, 0.37]) # alpha, beta, gamma_links
    ...
    def objective(params):
        ...
        for d in data:
            ...
            # interaction term: only for links (C > 1)
            interaction = gamma * d['K'] if d['C'] > 1 else 0.0
            ln_m_pred = eta * kappa * (d['V'] + alpha * d['T'] + beta * d['S'] + interaction) + B
    ...
    bounds = []
    for _ in sectors:
        bounds.extend([(0.1, 50.0), (-50.0, 50.0)])
    bounds.extend([(0.0, 2.0), (0.0, 2.0), (-1.0, 1.0)]) # alpha, beta, gamma
    ...
    print("Optimization Results (Interaction Gamma * K for Links):")
    print(f"Success: {res.success}")
    print(f"Final MAE: {res.fun:.4f}%")
    print(f"Alpha (Twist): {alpha:.4f}")
    print(f"Beta (Sig):   {beta:.4f}")
    print(f"Gamma (K, Links):  {gamma:.4f}")
    print("-" * 30)
    ...
    results = {
        ...
        "task_name": "Interaction Energy Term Initial Fitting (Links Only)",
        "computed_values": {
            ...
            "sectors": {s: {"eta": etas[s], "intercept": intercepts[s]} for s in sectors},

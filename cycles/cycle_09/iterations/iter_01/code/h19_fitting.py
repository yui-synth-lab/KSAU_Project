...
def main():
    ...
    # Initial guess: eta and B from SSoT, alpha=0.1, beta=0.1
    # Sector specific gamma
    model_config = consts['phase_viscosity_model']
    x0 = []
    for s in sectors:
        cfg = model_config['sectors'][s]
        x0.extend([cfg['eta'], cfg['intercept'], 0.0]) # eta, intercept, gamma
    ...
    def objective(params):
        ...
        for d in data:
            ...
            # ln_m_pred = eta * kappa * (V + alpha*T + beta*S + gamma*K) + B
    ...
    # Use bounds to keep eta positive and alpha/beta/gamma reasonable
    bounds = []
    for _ in sectors:
        bounds.extend([(0.1, 100.0), (-50.0, 50.0), (-10.0, 10.0)]) # eta, intercept, gamma
    bounds.extend([(0.0, 1.0), (0.0, 1.0)]) # alpha, beta
    
    res = minimize(objective, x0, method='Nelder-Mead', bounds=bounds, options={'maxiter': 10000})
    ...
    for d in data:
        ...
        print(f"{d['name']:<12}: Err {err:6.2f}% (K={d['K']}, V={d['V']:.2f}, S={d['S']})")
    ...
    results = {
        ...
        "task_name": "Interaction Energy Term Initial Fitting",
        "computed_values": {

...
def compute_ln_m_pred(p, d, kappa, alpha_fixed=0.1, beta_fixed=0.1):
    ...
    # Testing an even simpler 5-parameter model to ensure DoF > 0 even in LOO-CV
    # 0: eta_all (shared for all particles)
    # 1: B_quarks (shared for all quarks)
    # 2: B_leptons
    # 3: B_bosons
    # 4: gamma (interaction for links)
...
def main():
    ...
    results = {
        "iteration": "5", "hypothesis_id": "H19", "timestamp": time.strftime('%Y-%m-%dT%H:%M:%S'),
        "task_name": "LOO-CV Overfitting Check (Simplified 5-Param Model)",
        "computed_values": {

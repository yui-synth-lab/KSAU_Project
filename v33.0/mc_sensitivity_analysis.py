...
# -*- coding: utf-8 -*-
"""
KSAU v33.0 Session 3 - MC Sensitivity Analysis (ng.md #1 response)
====================================================================
Purpose:
  Quantitatively verify that the N^{1/4}/r_s MC result (p > Bonferroni threshold)
  is robust across three different sampling ranges:
    (A) standard:  [50,  500] Mpc  (original)
    (B) wide:      [30, 1000] Mpc
    (C) narrow:    [80,  300] Mpc

This resolves ng.md Indiciation #1 (CRITICAL):
  WARNING #2 partial-address: "range rationale documented" only.
  Now: actual MC runs with alternative ranges, results recorded.

SSoT: All constants from v6.0/data/*.json
"""
...
sys.stdout.write("=" * 65 + "\n")
sys.stdout.write("KSAU v33.0 Session 3 - MC Sensitivity Analysis\n")
sys.stdout.write("WARNING #2 response: Quantitative range sensitivity\n")
sys.stdout.write("=" * 65 + "\n")
...
sys.stdout.write(f"  n_max      = {N_MAX_RS}\n")
...
sys.stdout.write(f"{'Range':<22}  {'RS_MIN':>7}  {'RS_MAX':>7}  {'hits':>6}  "
...
for label, rs_min, rs_max in ranges:
    ...
    sys.stdout.write(f"{label:<22}  {rs_min:>7.1f}  {rs_max:>7.1f}  {hits:>6d}  "
                     f"{p:>8.5f}  {sig05:>7}  {sig0024:>9}  {verdict}\n")

sys.stdout.write("\n")
sys.stdout.write("--- Sensitivity Summary ---\n")
...
if all_not_sig:
    ...
    sys.stdout.write("  WARNING #2 status: PARTIALLY ADDRESSED ->\n")
    sys.stdout.write("    Range rationale: documented.\n")
    sys.stdout.write("    Quantitative sensitivity: COMPLETED (this script).\n")
    sys.stdout.write("  RESOLVED (both components satisfied).\n")
else:

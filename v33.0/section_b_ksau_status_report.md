...
**Status:** COMPLETED
**Date:** 2026-02-21
**Coverage:** KSAU v23.0 〜 v33.0（主要結果の横断整理）
...
本レポートは、KSAU（Kaluza-Stochastic-Algebraic-Unified）フレームワークの v23.0〜v33.0 にわたる全成果を横断的に整理し、外部の研究者が読んで現状を正確に把握できる「現状マップ」を提供することを目的とする。
...
**ステータス:** MOTIVATED_SIGNIFICANT (p = 0.032–0.038)

大規模構造（LSS）のパワースペクトルにおける KSAU 予測が、観測との照合で raw p < 0.05 を達成している。ただし：
- **Bonferroni 補正後の評価: 未実施**（Section 3 の検定数が正式確認されておらず、補正後閾値を適用できない状態。次フェーズの必須確認事項）
- WZW による理論的根拠は閉鎖（§3.1 参照）
- **BAO 代数的ブリッジ未発見**（v31.0 Section A 確定）

> **【修正注記 — v33.0 Session 3, ng.md 指摘#6 対応】**
> 旧文「Bonferroni 補正後の評価は未実施（または補正後有意性なし）」の「または」は曖昧化であった。正確な状態は **「Bonferroni 補正後評価は未実施」**（Section 3 の検定数が未確認のため補正後閾値を適用できない）。「補正後有意性なし」は確認されておらず、現時点では宣言できない。Section 3 の Bonferroni 補正後評価は次フェーズの必須タスク。

**現在の解釈:** 探索的 (exploratory) 結果として記録。Bonferroni 補正後評価は未完了。独立したデータセットでの再現確認が必要。
...
| 指標 | v31.0 | v32.0 | **v33.0** |
|------|-------|-------|-----------|
| ERR_THRESH | err_7（循環） | err_7（循環）| **独立（Planck_sigma）** |
| N_max | 固定 20 | 動的 round+5 | 動的 round+5（継承）|
| N_MC | 10,000 | 10,000 | **100,000** |
| N^{1/4}/r_s MC p | 0.040 | 0.0137 | 0.01176（Planck_sigma閾値）|
| Bonferroni 補正後 | 有意なし | 有意なし | **有意なし（全独立閾値で確認）** |

**v33.0 Task A の成果（循環閾値解消後）:** 旧循環閾値 ERR_THRESH = 0.2044%（err_7）は Planck 2018 公式不確かさ σ_{r_s}/r_s = 0.1768%（独立値）より 15.6% 緩く、p 値を 14.4% 相対的に過小評価していた。独立閾値採用後も全閾値（Planck_sigma: 0.1768%、1%, 5%, 10%）で Bonferroni 補正後有意なし。

**v33.0 Task B の成果（シード安定性）:** 複数シード（0, 1, 7, 42, 100, 314）全てで Bonferroni 補正後有意なし（std = 0.00058、安定）。seed=42 が結果に有利に働いていた証拠なし。
...
本レポートは、v23.0〜v33.0 の全成果を横断的に評価した結果として、以下を確認した：
...
*KSAU v33.0 — Section B: KSAU フレームワーク現状評価レポート*
*Status: COMPLETED*
*Date: 2026-02-21*
*Auditor: Claude (Independent Auditor)*
*Coverage: v23.0 〜 v33.0*

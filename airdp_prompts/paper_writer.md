...
2. {SSOT_DIR}/constants.json を読み込み、使用する定数・統計結果を確認する
...
[理論フレームワークの必要最小限の説明。数式は SSoT の値を使用]

---

## 3. Methods

[検証手法。統計的手法（p値、Bonferroni補正、FPR等）を明示。使用した実データの出典と前処理手順を記述]

---

## 4. Results

[定量的な結果のみ。R²、p値、FPR等を SSoT から引用。合成データによる結果は含めないこと]
...
brief.md の「図の仕様」セクションに記載された必須図について、それぞれ Python スクリプトを作成してください。

1. 出力先ディレクトリ: `{DRAFT_PATH}` と同階層の `figures/` ディレクトリ（なければ作成）
2. 各スクリプトは自己完結型（SSoT の JSON を直接読み込み、matplotlib で PNG を出力）
3. ファイル名規則: `fig_01_mass_formula.py`, `fig_02_scorecard.py`, ...
4. スクリプト実行後、draft 本文中で `![Fig. 1: ...](figures/fig_01_mass_formula.png)` として参照
5. 全ての数値は SSoT から読み込むこと（ハードコード禁止）
6. カラーパレットは色覚多様性対応（viridis 系推奨）
7. Revision 2 以降は、査読コメントで図の修正が指示された場合のみスクリプトを更新する
...
* **合成データによる結果を含めない。** 全ての結果は実データに基づくものでなければならない。データの出典と前処理手順を Results セクションまたは Methods セクションに明記すること。
``

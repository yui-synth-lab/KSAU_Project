...
```
チェックリスト:
□ コードが `from ksau_ssot import SSOT` を使っているか？（直接 Path/open でファイルを開いていないか）
□ `Path("E:/...")` `Path("./ssot")` 等のパスがコード内に存在しないか？（1件でも即却下）
...
### Step 6: roadmap.md のイテレーション割り当て表を更新し、完了判定を行う

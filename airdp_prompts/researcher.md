...
サイクル中に新しい定数が必要になった場合、以下を明記してください。
- 変数名: [例: PROPOSED_ETA]
- 暫定値: [数値]
- 単位: [例: MeV, dimensionless]
- 理論的根拠・導出元: [具体的に]
...
- 定数のハードコード（例: `3.14159` やドメイン固有の定数値など）
  - **例外**: 新しい定数が必要な場合は、コード内で `PROPOSED_X = <value>  # SSoT追加提案中` というコメントを付与し、かつ `researcher_report.md` の「SSoT追加提案」セクションに記載がある場合に限り、一時的にリテラル使用を許可する。
